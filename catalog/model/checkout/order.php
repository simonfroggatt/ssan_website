...
class ModelCheckoutOrder extends Model {
	public function addOrder($data) {
		$this->db->query("INSERT INTO `" . DB_PREFIX . "order` SET invoice_prefix = '" . $this->db->escape($data['invoice_prefix']) . "', store_id = '" . (int)$data['store_id'] . "', store_name = '" . $this->db->escape($data['store_name']) . "', store_url = '" . $this->db->escape($data['store_url']) . "', customer_id = '" . (int)$data['customer_id'] . "', customer_group_id = '" . (int)$data['customer_group_id'] . "', firstname = '" . $this->db->escape($data['firstname']) . "', lastname = '" . $this->db->escape($data['lastname']) . "', email = '" . $this->db->escape($data['email']) ."', company = '" . $this->db->escape($data['company']) . "', telephone = '" . $this->db->escape($data['telephone']) . "', fax = '" . $this->db->escape($data['fax']) . "', custom_field = '" . $this->db->escape(isset($data['custom_field']) ? json_encode($data['custom_field']) : '') . "', payment_firstname = '" . $this->db->escape($data['payment_firstname']) . "', payment_lastname = '" . $this->db->escape($data['payment_lastname']) . "', payment_company = '" . $this->db->escape($data['payment_company']) . "', payment_telephone = '" . $this->db->escape($data['payment_telephone']) . "', payment_email = '" . $this->db->escape($data['payment_email']) . "', payment_address_1 = '" . $this->db->escape($data['payment_address_1']) . "', payment_address_2 = '" . $this->db->escape($data['payment_address_2']) . "', payment_city = '" . $this->db->escape($data['payment_city']) . "', payment_postcode = '" . $this->db->escape($data['payment_postcode']) . "', payment_country = '" . $this->db->escape($data['payment_country']) . "', payment_country_id = '" . (int)$data['payment_country_id'] . "', payment_zone = '" . $this->db->escape($data['payment_zone']) . "', payment_zone_id = '" . (int)$data['payment_zone_id'] . "', payment_address_format = '" . $this->db->escape($data['payment_address_format']) . "', payment_custom_field = '" . $this->db->escape(isset($data['payment_custom_field']) ? json_encode($data['payment_custom_field']) : '') . "', payment_method = '" . $this->db->escape($data['payment_method']) . "', payment_code = '" . $this->db->escape($data['payment_code']) . "', shipping_firstname = '" . $this->db->escape($data['shipping_firstname']) . "', shipping_lastname = '" . $this->db->escape($data['shipping_lastname']) . "', shipping_company = '" . $this->db->escape($data['shipping_company']) . "', shipping_telephone = '" . $this->db->escape($data['shipping_telephone']) . "', shipping_email = '" . $this->db->escape($data['shipping_email']) ."', shipping_address_1 = '" . $this->db->escape($data['shipping_address_1']) . "', shipping_address_2 = '" . $this->db->escape($data['shipping_address_2']) . "', shipping_city = '" . $this->db->escape($data['shipping_city']) . "', shipping_postcode = '" . $this->db->escape($data['shipping_postcode']) . "', shipping_country = '" . $this->db->escape($data['shipping_country']) . "', shipping_country_id = '" . (int)$data['shipping_country_id'] . "', shipping_zone = '" . $this->db->escape($data['shipping_zone']) . "', shipping_zone_id = '" . (int)$data['shipping_zone_id'] . "', shipping_address_format = '" . $this->db->escape($data['shipping_address_format']) . "', shipping_custom_field = '" . $this->db->escape(isset($data['shipping_custom_field']) ? json_encode($data['shipping_custom_field']) : '') . "', shipping_method = '" . $this->db->escape($data['shipping_method']) . "', shipping_code = '" . $this->db->escape($data['shipping_code']) . "', comment = '" . $this->db->escape($data['comment']) . "', total = '" . (float)$data['total'] . "', affiliate_id = '" . (int)$data['affiliate_id'] . "', commission = '" . (float)$data['commission'] . "', marketing_id = '" . (int)$data['marketing_id'] . "', tracking = '" . $this->db->escape($data['tracking']) . "', language_id = '" . (int)$data['language_id'] . "', currency_id = '" . (int)$data['currency_id'] . "', currency_code = '" . $this->db->escape($data['currency_code']) . "', currency_value = '" . (float)$data['currency_value'] . "', ip = '" . $this->db->escape($data['ip']) . "', forwarded_ip = '" .  $this->db->escape($data['forwarded_ip']) . "', user_agent = '" . $this->db->escape($data['user_agent']) . "', accept_language = '" . $this->db->escape($data['accept_language']) . "', date_added = NOW(), date_modified = NOW(), date_paid = NOW(), payment_due_date = NOW()");

		$order_id = $this->db->getLastId();
		$need_svg_convert = false;
		// Products
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				$sql = "INSERT INTO " . DB_PREFIX . "order_product SET order_id = '" . (int)$order_id . "', product_id = '" . (int)$product['product_id'] . "', name = '" . $this->db->escape($product['name']) . "', model = '" . $this->db->escape($product['model']) . "', quantity = '" . (int)$product['quantity'] . "', price = '" . (float)$product['price'] . "', total = '" . (float)$product['total'] . "', tax = '" . (float)$product['tax'] . "', reward = '" . (int)$product['reward'] . "'" . ",  product_variant_id = '".$product['product_variant_id']."'".", size_name = '" . $product['size_name'] . "' , material_name = '" . $product['material_name'] ."', is_bespoke = '" . $product['is_bespoke'] ."'";


				$this->db->query($sql);

				$order_product_id = $this->db->getLastId();  //no convinced this is correct

				foreach ($product['option'] as $option) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "order_option SET order_id = '" . (int)$order_id . "', order_product_id = '" . (int)$order_product_id . "', product_option_id = '" . (int)$option['product_option_id'] . "', product_option_value_id = '" . (int)$option['product_option_value_id'] . "', name = '" . $this->db->escape($option['name']) . "', `value` = '" . $this->db->escape($option['value']) . "', `type` = '" . $this->db->escape($option['type']) . "'" );
				}

				if($product['svg_json'] != null) {
					$need_svg_convert = true;
					//now add the svg code in here for bespoke images
					$sql = "INSERT INTO ". SSAN_DB_PREFIX ."order_bespoke_image SET order_product_id = '" .(int)$order_product_id ."' , svg_export = '" . $this->db->escape($product['svg_export']) ."', svg_json = '" . $this->db->escape($product['svg_json']) . "', sign_text = '" . $this->db->escape($product['svg_bespoke_texts']) . "', product_var_id = '" . (int)$product['product_variant_id'] . "'";
					$this->db->query($sql);
					$bespoke_product_id = $this->db->getLastId();
					$bepoke_model_id = (int)$order_id . "-". (int)$bespoke_product_id;
					$this->db->query("UPDATE " . DB_PREFIX . "order_product SET model = '#" . $bepoke_model_id ."' WHERE order_product_id = '" . (int)$order_product_id . "'");
					$this->db->query("UPDATE " . SSAN_DB_PREFIX . "order_bespoke_image SET png_url = '" . $bepoke_model_id .".png' WHERE id = '" . (int)$bespoke_product_id . "'");

					$bespoke_symbol = explode(",",$product['svg_bespoke_images']);
					foreach($bespoke_symbol as $bespoke_symbol_id) {
							$sql = "INSERT INTO ". SSAN_DB_PREFIX . "bespoke_image_symbols SET bespoke_id = '". (int)$bespoke_product_id . "', symbol_id = '". (int)$bespoke_symbol_id. "'";
							$this->db->query($sql);
						}
					}
			}
		...
		if($need_svg_convert){
			//lets run our python svg converter in here!!
			exec('/usr/bin/python3.6 /var/www/vhosts/safetysignsandnotices.co.uk/httpdocs/system/scripts/bespokeconvert.py -order '.$order_id. ' > /dev/null &');
		}
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false, $payment_complete = false) {
		...
		if ($order_info) {
			...
			//SSAN - when the order is complete (id 99) we set the paid status to 2;
			if($payment_complete)
				$this->db->query("UPDATE `" . DB_PREFIX . "order` SET payment_status_id = 2, date_modified = NOW() WHERE order_id = '" . (int)$order_id . "'");

	...
	public function addOrder($data) {
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				if($product['is_bespoke'] == 1) {
					$need_svg_convert = true;
					//now add the svg code in here for bespoke images
					$sql = "INSERT INTO ". SSAN_DB_PREFIX ."order_bespoke_image SET order_product_id = '" .(int)$order_product_id ."' , svg_export = '" . $this->db->escape($product['svg_export']) ."', svg_json = '" . $this->db->escape($product['svg_json']) . "', sign_text = '" . $product['svg_bespoke_texts'] . "', product_var_id = '" . (int)$product['product_variant_id'] . "'";
					$this->db->query($sql);

          ...
          <tr id="<?php echo $product_variant_data['id']; ?>" data-uniqueid="<?php echo $product_variant_data['id']; ?>">
            ...
            <?php foreach($product_variant_data['discount_array'] as $index => $discount_value){ ?>
              <td class="col-md-1 bulkcell_<?php echo $product_variant_data['id'];?> product-variant" data-variant-bulk-id="<?php echo $product_variant_data['id']?>_<?php echo $index; ?>">
                <?php echo number_format($discount_value * $tax_multiplier,2); ?>
              </td>
              <?php } ?>
                <td class="col-md-1">
                  <?php
      ...
      <thead>
        ...
        <tr>
          ...
          <?php foreach($bulk_discount_group_titles as $key => $bulk_discount_data) { ?>
            ...
            <?php } ?>
              <th data-field="qty" data-sortable="false" data-halign="center" data-align="center">QTY</th>
        </tr>
      </thead>
      <tfoot>

      if($is_bespoke) {
        <td colspan="<?php echo ($bulk_discount_price_count + 4);?>"><span id="vat-status-text"></span></td>
         }
        else {
          <tr>
            ...
            <td colspan="2">
              ...
              <button type="button" id="button-cart-table" data-loading-text="loading..." class="btn btn-add-cart btn-lg btn-block"><?php

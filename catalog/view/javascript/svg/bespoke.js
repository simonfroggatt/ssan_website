var bespokeSign = function(drawObject, options) {
  ...
  this.buildSign = function(clearSign, whatChanged, f) {
    ...
    svgDrawObj.drawSign(drawVars, clearSign);
  ...
  this.setSize = function(width, height, orientation) {
    var newOrientation;
    if (orientation === -1) {
      if (width > height) {
        newOrientation = 1;
  ...
  this.setSymbol = function(imageInfo, imagePosition, redraw) {
    bespokeConstructor.setSymbol(imagePosition, IMAGEDIRPATH + imageInfo['svg_path'], imageInfo);
    if (redraw) {
      this.buildSign(true, '', false); //function(clearSign = false, whatChanged = '', f = false)
    }
  };
...
*/
  ...
  this.changeSignSize = function(width, height, orientation, panelID, textBlocks) {
    var newOrientation,
        blSizeChanged,
        oldDims,
        drawVars;
  ...
  this.exportToSVGThumb = function(currentSVGObj) {
    ...
    this.buildSignForExport(true, true, [100, 100], 'px', null, false);  //function(addBorder = false, addFrame = false, drawingFrame = [200, 200], measurementUnits = 'px', drawObjectHolder = null, f = false)
  ...
  this.exportToSVG = function(currentSVGObj) {
    ...
    bespokeConstructor.loadFromJSON($.parseJSON(SVGcode));
    this.buildSignForExport(false, false, [200,200], 'mm',null, false);  //function(addBorder = false, addFrame = false, drawingFrame = [200, 200], measurementUnits = 'px', drawObjectHolder = null, f = false)
    var SVGCode = svgDrawObj.exportToSVGThumb();
    //  console.log(SVGCode)
    return SVGCode;
    //  this.buildSignForExport(false);
  ...
  this.buildSignForExport = function(addBorder, addFrame , drawingFrame, measurementUnits, drawObjectHolder, f) {
  ...
  var testsize = function(changeBy, textPanelID, textPanelLineID) {
    ...
    var newSize = svgDrawObj.ChangeTextSize(textPanelID, textPanelLineID, changeBy, null);
  ...
  var reDrawText = function(textPanelID, textPanelLineID) {
    //  bespokeConstructor.createSign()

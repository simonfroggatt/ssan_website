...
var bespokeDrawer = function(drawObject, options) {
  ...
  this.moveTextVertically = function(panelID, blockID, changeBy) {
    ...
    if(changeBy < 0){  //moving up the sign
      if(textLines.y === textLinebounds.y){ //already at the highest point
        redraw = false;
      }else {
        if (newY < textLinebounds.y) { //this would move it too far
          newY = textLinebounds.y;
        }
      }
      ...
      if((textLinebounds.y + textLinebounds.height) === (textLines.linesHeight + textLines.y) ) {
        redraw = false;
      } else {
        if ((newY + textLines.linesHeight) > (textLinebounds.y + textLinebounds.height)) { //moved too far down, so set to the max it can move
          newY = (textLinebounds.y + textLinebounds.height) - textLines.linesHeight;
        }
      }
    }

...
function setSelectedVariantPageInformation(productVariantSelectedInfo) {
  ...
  setSizeMaterials(productVariantSelectedInfo.product_size_id);
...
function changePricesReVAT(addVAT)
{
  ...
  $.each(tableData, function(index, value){
    ...
    for (i = 0; i < bulkDiscountCount; i++) {
      ...
      firstnumber = parseFloat(cellToRecalc.html());

      if(addVAT) {
        newval = $.number(firstnumber / tax_multiplier, 2);
        newval += '<br><span class="tabletax">(inc VAT ' + firstnumber.toFixed(2) + ')<span>';
      }
      else {
        newval = $.number(firstnumber * tax_multiplier,2);
        newval += '<br><span class="tabletax">(ex VAT ' + firstnumber.toFixed(2) + ')<span>';
      }
...
function updateProductDetails(fullupdate)
{
...
//get size and material combinations
  ...
  var new_price_arr = getBulkPricingValue(ssan_var_info.discount_array,qty);
  var new_price = new_price_arr[0];
  var new_price_vat = new_price_arr[1];
  $('#priceDropDown').html(new_price);
  var tax_string = "";

  if(parseFloat(new_price) > parseFloat(new_price_vat)) {
    tax_string = "(ex VAT £";
  }
  else {
    tax_string = "(inc VAT £";
  }
  $('#priceDropDownTax').html(tax_string+new_price_vat+")");
...
function switchImage(ssan_var_info)
{
  ...
  var imageSrc = defaultProductImage;
  if(ssan_var_info.alternative_image !== null)
  {
    imageSrc = ssan_var_info.alternative_image;
  }
...
function getBulkPricingValue(bulkPricingArray, qtySelectec)
{
    ...
    if(currentVatStatus == 1) {
      newval[1] = price;
      newval[0] = $.number(price * tax_multiplier,2);

    }
    else {
      newval[0] = price;
      newval[1] = $.number(price * tax_multiplier,2);
...
function makeNiceStockStatus(stockDays)
{
  ...
  default:   prettyStockDays = stockDays - parseInt(daySpan) + '-' + stockDays + ' days';  break;
  }

  return prettyStockDays;
}

function setPreloadedVariantID(variantID)
{
  var sizeid = 0;
  var materialid = 0;
  ...
  $.each(productVariantInfomationArray, function(index, item) {
    if ( (item.id == variantID)) {
      ...
      materialid = item.product_material_id
      return true;
    }
  });
  if( (sizeid > 0) && (materialid > 0) ){
    $('#posize').val(sizeid)
    setSizeMaterials(sizeid);
    $('#pomaterial').val(materialid)
    updateProductDetails(true)
    setTableHighlight();
  }
}

...
class ControllerExtensionPaymentPPPro extends Controller {
	public function index() {
		...
		$data['name_for_card'] = $this->session->data['payment_address']['firstname'] . ' ' . $this->session->data['payment_address']['lastname'];
	...
	//	$request .= '&EXPDATE=' . urlencode($this->request->post['cc_exp_month'] . $this->request->post['cc_exp_year']);
	//	$request .= '&CVV2=' . urlencode($this->request->post['cc_cvc']);
		...
		if (($response_info['ACK'] == 'Success') || ($response_info['ACK'] == 'SuccessWithWarning')) {
			...
			$this->model_checkout_order->addOrderHistory($this->session->data['order_id'], $this->config->get('pp_pro_order_status_id'), $message, false, false, true);
			//SSAN - set payment status

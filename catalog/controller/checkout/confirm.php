	...
	public function index() {
		...
		if (!$redirect) {
			...
			if ($this->customer->isLogged()) {
				...
				$order_data['company'] = $this->session->data['payment_address']['company'];
			...
			{
				$order_data['payment_telephone'] = $this->session->data['guest']['telephone'];
				$order_data['payment_email'] = $this->session->data['guest']['email'];
			}
			else {
			...
			if ($this->cart->hasShipping()) {
				...
				if(isset($this->session->data['guest']))
				{
					...
					$order_data['shipping_email'] = $this->session->data['guest']['email'];
					$order_data['shipping_telephone'] = $this->session->data['guest']['telephone'];
				}
				else {
			...
			//ssan
			//create an account in the background without the customer knowing,
			if(isset($this->session->data['guest']))
			{
				$this->load->model('account/customer');
				$email_count = $this->model_account_customer->getTotalCustomersByEmail($order_data['email']);
				if($email_count > 0)
				{
					...
					$customer_data['password'] = $order_data['firstname'];
		...
		//THIS IS WHERE WE MAKE THE CALL TO PYTHON TO DO THE COOL SHIT !!!
